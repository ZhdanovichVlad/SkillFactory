...
def print_game_field(game_field):
    ''' Function prints the game field '''
    ...
    for i in range(3):
        print(
            f'{i+1}   | {game_field[i][0]} | {game_field[i][1]} | {game_field[i][2]} | ')
        print("  --------------- ")
...
def main():
    ...
    print('''Добро пожаловать в консольную игру крестики нолики от Владислава Ждановича''')
    print('Начнем игру')
    ...
    while True:
        ...
        step = input(
            f"Ход {stage}. Введите через пробел кординаты на оси X и Y,напоминание от 1 до 3 \n")

        try:
            coordinates = list(
                map(lambda x: x - 1, map(int, step.strip(" ").split(" "))))
        except ValueError:
            print(
                "Ошибка введены буквы или символы, или стоит лишний пробел, попробуйте ещё раз")

        if len(coordinates) != 2:
            print("Введенные координаты не соответствуют шаблону")
            continue
        ...
        if game_field[coordinates[0]][coordinates[1]] == "X" or game_field[coordinates[0]][coordinates[1]] == "0":
            print(
                f"Данное поле занято, тут стоит {game_field[coordinates[0]][coordinates[1]]} введите данные заново")
            continue
